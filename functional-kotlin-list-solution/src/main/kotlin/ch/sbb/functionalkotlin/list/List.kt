...
sealed class List<out A> {
    ...
    fun drop(n: Int): List<A> = when (this) {
        Nil -> this
    ...
    fun <B> foldLeft(acc: B, f: (B) -> (A) -> B): B = foldLeft(this, acc, f)
    ...
    tailrec fun <B> coFoldRight(identity: B, f: (A) -> (B) -> B): B = foldLeft(this.reverse(), identity, {b: B -> { a: A -> f(a)(b) }})
    ...
    companion object {
        ...
        fun <A, B> foldRight(list: List<A>, acc: B, f: (A) -> (B) -> B): B =
                when (list) {
                    Nil -> acc
                    is Cons -> f(list.head)(foldRight(list.tail, acc, f))
                }

        fun <A, B> foldLeft(list: List<A>, identity: B, f: (B) -> (A) -> B): B =
                when (list) {
                    Nil -> identity
